...
I've got the shortener running at `s.mfyz.app` and I'm planning to add more workers to the same repo. The monorepo setup is simple, one folder per worker, each with its own `wrangler.toml` and `package.json`. Deploy independently with `cd workers/url-shortener && wrangler deploy`.

If you want to build lightweight serverless apps without dealing with containers, cold starts, or infrastructure, Workers + D1 is a really solid combo.
