---
title: "Claude Code Loop: The Ultimate YOLO Mode"
...
date: 2025-10-28
slug: "claude-code-loop-autonomous-ai-coding"
...
This script both logs the outpus, as well as streams the output and makes it more readable using jq. So it's interesting to watch shell output, but it gets old, and after some point you just let it do its thing. Between each run, we print a big number in ASCII art (using figlet) to make it separated between runs.

## Key Features I Added

**Iteration control.** Instead of running forever, you set a max iteration count. This prevents your API bill from getting out of control while you're asleep.

**Structured logging.** Using `jq` to parse the JSON stream and format it nicely. You see exactly what tools Claude is using and what it's thinking.

**Clean interrupts.** Hit Ctrl+C and everything shuts down gracefully. No zombie processes.

**Formatted output.** Those ASCII art iteration numbers? Completely unnecessary but incredibly satisfying.

## The Claude Code Stream Format

One thing I learned: Claude Code can output in `stream-json` format, which gives you structured data about everything that's happening:

- Message starts and stops
- Content deltas (the actual text being generated)
- Tool calls (when Claude uses Read, Write, Bash, etc.)
- Thinking blocks

By piping this through `jq`, you can filter and format it however you want. I'm showing messages and tool calls, but filtering out the noise.

## Safety Considerations

Let me be super clear: this is very experimental. Do not use in your production code. Here's what you need to think about:

**Run it in a sandbox.** Seriously. Docker container, VM, something disposable. Claude has `--dangerously-skip-permissions` enabled, which means it can do pretty much anything.

**Watch your API or usage costs.** 50 iterations of Claude Sonnet can add up. Set reasonable limits and check your usage. I am on Max plan and running loops on 2 projects at the same time runs me out of rate limit window less than an hour.

**Review the changes.** Don't blindly merge what comes out of this. The agent can make mistakes, introduce bugs, or go in unexpected directions. Especially in this loop mode, with high-level prompts, it can do way more than you asked, and unexpected stuff. Having a preview branch system (i.e Vercel preview deployments) helps a lot to test, and see what it actually produced. Doing PR review is must, but imagine the amount of changes and code produced out of 40-50 runs overnight... It won't be fun to sit and scroll code.

**Version control is essential.** You want to be able to roll back easily. Git is your safety net here. Consider 1/4 of the time you'll just throw it away. If you are not using version control, get out of here :D
