...
export async function getPosts({
  ...
  const posts = await getCollection("blog", ({ data, id, slug }) => {
    ...
    if (!includeHidden && data.hidden) return false;
    ...
    // Language filtering
    if (language !== 'all') {
      // Check if the post is in the Turkish folder or has Turkish language in frontmatter
      const isTurkishPath = id.startsWith('tr/');
      const hasTurkishLang = data.lang === 'tr';
      const isTurkishPost = isTurkishPath || hasTurkishLang;

      // For English posts, exclude Turkish posts
      if (language === 'en') {
        return !isTurkishPost;
      }
      
      // For Turkish posts, only include Turkish posts
      if (language === 'tr') {
        return isTurkishPost;
      }
    }
 ...
 */
export async function getEnglishPosts(options: Omit<PostFilterOptions, 'language'> = {}): Promise<CollectionEntry<'blog'>[]> {
 ...
 */
export async function getTurkishPosts(options: Omit<PostFilterOptions, 'language'> = {}): Promise<CollectionEntry<'blog'>[]> {
 ...
 */
export async function getAllPosts(options: Omit<PostFilterOptions, 'language'> = {}): Promise<CollectionEntry<'blog'>[]> {
...
export async function getRelatedPosts({ 
  ...
  let relatedPosts = await getPosts({ 
    ...
    limit: requireMatchingTag && tags.length > 0 ? Math.max(limit * 3, 20) : limit
  });
