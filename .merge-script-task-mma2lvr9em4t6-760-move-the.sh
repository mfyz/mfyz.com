#!/bin/bash
exec > "/home/remote/Development/mfyz.com/.merge-log-task-mma2lvr9em4t6-760-move-the.txt" 2>&1
echo "[merge] Starting safe merge of task-mma2lvr9em4t6-760-move-the into main"

cleanup_fail() {
  rm -f "/home/remote/Development/mfyz.com/.nox-merge.lock" 2>/dev/null || true
  rm -f "/home/remote/Development/mfyz.com/.merge-log-task-mma2lvr9em4t6-760-move-the.txt" 2>/dev/null || true
...
echo '{"branch":"task-mma2lvr9em4t6-760-move-the","timestamp":'$(date +%s000)',"pid":'$$'}' > "$LOCK_FILE"
...
if [ -n "$(git -C "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" status --porcelain)" ]; then
  echo "[merge] Auto-committing worktree changes"
  git -C "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" add -A
  git -C "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" commit -m "Auto-commit before merge" || true
fi
...
if ! git -C "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" merge "main" --no-edit; then
  CONFLICT_FILES=$(git -C "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" diff --name-only --diff-filter=U 2>/dev/null)
  if [ -n "$CONFLICT_FILES" ]; then
    echo "[merge] Conflicts detected in worktree, spawning AI resolver..."
    CONFLICT_LIST=$(echo "$CONFLICT_FILES" | tr '\n' ', ' | sed 's/,$//')

    RESOLVE_PROMPT="You are resolving git merge conflicts in a worktree at: /home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the
...
IMPORTANT: Do NOT abort the merge. Resolve all conflicts and commit."
    ...
    TASK_JSON=$(node "/home/remote/.claude/my_tools/taskman.js" exec "resolve-merge-task-mma2lvr9em4t6-760-move-the" --prompt "$RESOLVE_PROMPT" --cwd "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" --source "nox-merge" 2>/dev/null)
    ...
    if [ -n "$TASK_ID" ]; then
      ...
      if [ "$STATUS" != "completed" ]; then
        ...
        git -C "/home/remote/Development/mfyz.com-worktrees/task-mma2lvr9em4t6-760-move-the" merge --abort 2>/dev/null || true
        cleanup_fail
      fi
...
echo "[merge] Merging task-mma2lvr9em4t6-760-move-the into main"
git -C "/home/remote/Development/mfyz.com" checkout "main"
if ! git -C "/home/remote/Development/mfyz.com" merge "task-mma2lvr9em4t6-760-move-the" --no-ff -m "Merge worktree: task-mma2lvr9em4t6-760-move-the"; then
...
"/home/remote/.local/bin/wtp" remove --with-branch "task-mma2lvr9em4t6-760-move-the" 2>/dev/null || {
  git -C "/home/remote/Development/mfyz.com" worktree remove --force "task-mma2lvr9em4t6-760-move-the" 2>/dev/null || true
  git -C "/home/remote/Development/mfyz.com" branch -d "task-mma2lvr9em4t6-760-move-the" 2>/dev/null || true
}

echo "[merge] Updating linked todo"
if [ -d "/home/remote/life/Todos" ]; then
  for f in "/home/remote/life/Todos"/*.md; do
    if grep -q "worktree_branch: task-mma2lvr9em4t6-760-move-the" "$f" 2>/dev/null; then
...
rm -f "/home/remote/Development/mfyz.com/.worktree-reviews/task-mma2lvr9em4t6-760-move-the.json" 2>/dev/null || true
